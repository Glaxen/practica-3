                    ...
                    <tr>
                        ...
                        <td>
                            <div class="row">
                                {{-- botton de editar --}}
                                <div class="col-6">
                                    <button id="{{$model->id}}" type="button" class="btneditbrandmodel btn btn-primary">
                                        <i class="fas fa-solid fa-pen"></i>
                                    </button>
                                </div>
                                {{-- boton de borrar  --}}
                                <div class="col-6">
                                    <form class="frmDelete" action="{{ route('admin.brandsmodel.destroy', $model->id) }}" method="post">
                                        @method('delete')
                                        @csrf
...
<script>
    $(document).ready(function() {
        ...
        $.ajax({
            url:"{{ route('admin.brandsmodel.create') }}",
            type: "GET",
        ...
        $('.btneditbrandmodel').click(function() {
            var id = $(this).attr('id');
            $.ajax({
                url:"{{ route('admin.brandsmodel.edit','_id') }}".replace('_id',id),
                type: "GET",
                success: function (response) {
                    $('#modalbrand .modal-body').html(response);
                    $('#modalbrand #exampleModalLabel').html('Actualizar modelo');
                    $('#modalbrand').modal('show');
                },
                error: function(xhr, status, error) {
                    console.error('Error en la solicitud AJAX:', error);
